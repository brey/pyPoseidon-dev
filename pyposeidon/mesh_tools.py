...
def split_boundaries(m, chunk=1000, **kwargs):
    ...
    d = m[['id','node','type']].to_dataframe() # get boundary info
    ...
    d_new = pd.concat([d,new]).sort_values(["type", "id", "node"]).reset_index(drop=True)
    
    d_new.index.name='bnodes'
    
    mnew = m.drop_vars(['node','type','id']).drop_vars('bnodes') # drop old
...
def populate(dem,perms,m,coastlines,buffer=0.):
    ...
    #    de = dem.sel(lon=slice(blon1,blon2)).sel(lat=slice(blat1,blat2))
        ...
        #subset mesh
        indices_of_nodes_in_bbox = np.where((m.SCHISM_hgrid_node_y >= lat1 - buffer/2) &
                                        (m.SCHISM_hgrid_node_y <= lat2 + buffer/2) & 
                                        (m.SCHISM_hgrid_node_x >= lon1 - buffer/2) & 
                                        (m.SCHISM_hgrid_node_x <= lon2 + buffer/2))[0]
...
def fill_dem_on_mesh(mesh_file,dem_files,coastlines):
    ...
    m0 = pm.set(type='tri2d', mesh_file=mesh_file)
    m_ = m0.Dataset
